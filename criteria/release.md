...
The files you write are `.agy/runs/<run-id>/RELEASE_PLAN.md`,
`.agy/runs/<run-id>/phases/RELEASE/verdict`, the changelog, and any
version-bearing file the facts below name. Nothing else.
...
`.agy/runs/<run-id>/RELEASE_FACTS.md` was written for you by
`scripts/check-release.sh` immediately before you were dispatched. You cannot
run `git`, so that file is your only account of the repository's state — the
branch you are on, whether it is the release branch, whether a remote exists,
whether the tree is clean, what tags exist and in what format, and the version
the script proposes next.
...
version. Write `.agy/runs/<run-id>/RELEASE_PLAN.md` saying only that no facts
file was provided, so no release could be prepared, and return the `BLOCKED`
verdict.
...
`.agy/runs/<run-id>/CHANGES.md` (what this task changed),
`.agy/runs/<run-id>/DISCOVERY.md`, `.agy/runs/<run-id>/QA_REPORT.md`, the
existing changelog, and any issue or spec the brief names.
...
- **The bump size is a judgement**, and the facts file names the alternatives.
  ...
  `.agy/runs/<run-id>/CHANGES.md` and the proposal disagree — a breaking change
  against a proposed patch bump — say so plainly and propose the version you
  believe is right, with the reason. You are recommending, not overruling: both
  versions go in the plan, and the human picks.
...
- **Only what changed in this task.** Take it from
  `.agy/runs/<run-id>/CHANGES.md` and the diff described there. Do not summarise
  the repository's whole history, and do not restate entries the file already
  has.
...
## 5. Write the plan — `.agy/runs/<run-id>/RELEASE_PLAN.md`
...
`.agy/runs/<run-id>/DISCOVERY.md`, and the QA verdict from
`.agy/runs/<run-id>/QA_REPORT.md` if one exists. A release proposed over a red
suite says so here, at the top of this section, in one sentence.

## 6. Verdict

End with one line, written to `.agy/runs/<run-id>/phases/RELEASE/verdict` and
printed as the last line of your output:

- `STATUS: PREPARED | File: .agy/runs/<run-id>/RELEASE_PLAN.md` — the changelog
  entry is written, any version files are updated, and the plan lists the
  commands that remain.
- `STATUS: BLOCKED | File: .agy/runs/<run-id>/RELEASE_PLAN.md` — you could not
  prepare the release. The plan says exactly what is missing and what decision
  is needed.

Do not write `.agy/runs/<run-id>/phases/RELEASE/status`. That file belongs to the
orchestrator's own tooling.
